-- =============================================================================
-- Autodox Platform - Consolidated Database Schema
-- Target: External Supabase instance (iqluzpzttzoaybbjvtsr)
...
-- 4. After completion, run the auth trigger separately (see bottom of file)
--
-- =============================================================================
...
CREATE TYPE public.template_status AS ENUM ('draft', 'published', 'disabled');
CREATE TYPE public.landing_base_layout AS ENUM (
  'hero_focused',
  'compliance_heavy',
  'trust_signal_dense',
  'minimal_enterprise',
...
-- Applications table for KYB data
...
-- Brands table for multi-tenant management
CREATE TABLE public.brands (
  ...
  active_template_id UUID REFERENCES public.landing_templates(id),
  applied_template_version INTEGER,
  template_applied_at TIMESTAMPTZ,
  template_applied_by UUID,
  previous_template_id UUID REFERENCES public.landing_templates(id),
  previous_template_version INTEGER,
  domain_status public.domain_status DEFAULT NULL,
  domain_verification_token TEXT DEFAULT NULL,
  domain_verified_at TIMESTAMP WITH TIME ZONE DEFAULT NULL,
  ssl_status TEXT DEFAULT NULL,
  cloudflare_hostname_id TEXT DEFAULT NULL,
  domain_error TEXT DEFAULT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Template activity log (immutable audit trail)
...
-- Portal configs for white-label theming
...
-- Tickets table for support center
...
-- Function to prevent activity log modification
...
-- Function to call edge function on application status change
-- IMPORTANT: URL updated for new Supabase project
...
BEGIN
  IF OLD.status = 'pending' AND NEW.status IN ('approved', 'rejected') THEN
    ...
    PERFORM extensions.http_post(
      url := 'https://iqluzpzttzoaybbjvtsr.supabase.co/functions/v1/notify-application-status',
      body := payload,
...
-- Activity log immutability triggers
...
-- -----------------------------------------------------------------------------
-- PART 11: RLS Policies - Landing Templates
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- PART 12: RLS Policies - Template Activity Log
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- PART 13: RLS Policies - Portal Configs
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- PART 15: RLS Policies - Contact Submissions
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- PART 17: Enable Realtime
-- -----------------------------------------------------------------------------

ALTER PUBLICATION supabase_realtime ADD TABLE public.applications;

-- -----------------------------------------------------------------------------
-- PART 18: Storage Bucket
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- PART 19: Seed Templates (60 total)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- 
-- IMPORTANT: After running this script, you must execute the following
-- command separately in the Supabase SQL Editor to create the auth trigger:
--
...
--
-- This cannot be included in this migration file because it references
-- the auth schema which requires special permissions.
-- =============================================================================
